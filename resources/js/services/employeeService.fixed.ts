// resources/js/services/employeeService.fixed.ts
...
export const employeeService = {
    ...
    getAllEmployees: async (): Promise<Employee[]> => {
        try {
            ...
            const response = await axios.get(`/api/employees?_=${timestamp}`);
    ...
    getEmployeeById: async (id: string, timestamp?: number): Promise<Employee | null> => {
        try {
            ...
            const url = timestamp ? `/api/employees/${id}?_=${timestamp}` : `/api/employees/${id}?_=${new Date().getTime()}`; // Always add cache busting
