        ...
        <div className="flex h-screen bg-gray-100">
            ...
            <div className={`flex flex-1 flex-col transition-all duration-300 ease-in-out ${isSidebarOpen ? 'ml-64' : 'ml-20'}`}>
                ...
                <main className="flex-1 overflow-y-auto bg-white p-6">
                    <h1 className="mb-6 text-3xl font-bold text-black">Employee Patients</h1>
                        ...
                        <table className="min-w-full divide-y divide-gray-200">
                            <thead className="bg-[#D4A5B8] text-black">
                                <tr>
                                    ...
                                    <th className="px-6 py-3 text-left text-xs font-medium tracking-wider text-black uppercase">Department</th>
                                    <th className="px-6 py-3 text-left text-xs font-medium tracking-wider text-black uppercase">Position</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-gray-200 bg-white">
                                {isLoading ? (
                                    ...
                                    filteredAndSortedEmployees.map((employee: EmployeeType) => (
                                        <tr
                                            key={employee.id}
                                            className="cursor-pointer hover:bg-gray-50"
                                            onClick={() => handleNavigation(`/consultation/employee/${employee.id}`)}
                                        >
                                            <td className="px-6 py-4 text-sm whitespace-nowrap text-black">{employee.id}</td>
                                            <td className="px-6 py-4 text-sm whitespace-nowrap text-black">{employee.name}</td>
                                            <td className="px-6 py-4 text-sm whitespace-nowrap text-black">{employee.age}</td>
                                            <td className="px-6 py-4 text-sm whitespace-nowrap text-black">{employee.gender}</td>
                                            <td className="px-6 py-4 text-sm whitespace-nowrap text-black">{employee.department}</td>
                                            <td className="px-6 py-4 text-sm whitespace-nowrap text-black">{employee.position}</td>
                                        </tr>
                                    ))
                                )}
                            </tbody>
